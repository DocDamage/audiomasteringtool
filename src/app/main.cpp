...
// Route the mastering call through Phase 5 and persist diagnostic
// provenance beside the project's render directory for inspection/restore.
...
// Keep the unattended desktop smoke test deterministic and independent of the
// optional, on-demand source-separation model. Real packaged model acceptance
// is exercised by the release checklist rather than this UI pipeline check.
[[nodiscard]] std::optional<MasteringRenderPair>
render_mastering_plan_stereo_self_test(
    amt::codec::ICodecService& codecs,
    const std::filesystem::path& canonical_input,
    const std::filesystem::path& output_directory,
    const amt::analysis::Phase1AnalysisReport& source_analysis,
    MasteringPlan& plan,
    std::string& error) {
  return render_mastering_plan(codecs, canonical_input, output_directory,
                               source_analysis, plan, error);
}
